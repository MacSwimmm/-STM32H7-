...
*					2.	可设置要显示的颜色，例如使用 LCD_SetColor(0xff0000FF) 设置为蓝色
*					3. 可设置对应的背景色，例如使用 LCD_SetBackColor(0xff000000) 设置为黑色的背景色
*					4. 使用示例 LCD_DisplayString( 10, 10, "1111KE") ，在起始坐标为(10,10)的地方显示字符串"1111KE"
*
***************************************************************************************************************/
...
*					2.	可设置要显示的颜色，例如使用 LCD_SetColor(0xff0000FF) 设置为蓝色
*					3. 可设置对应的背景色，例如使用 LCD_SetBackColor(0xff000000) 设置为黑色的背景色
*					4. 使用示例 LCD_DisplayChinese( 10, 10, "1111科技STM32") ，在坐标(10,10)显示字符串"1111科技STM32"
*
***************************************************************************************************************/
...
/**
 * @brief       在指定区域内填充指定颜色块（RGB屏软件绘制版本）
 * @param       sx,sy: 起始坐标
 * @param       ex,ey: 结束坐标  
 * @param       color: 要填充的颜色数组首地址（RGB565格式）
 * @retval      无
 */
...
void DMA2D_DrawPixel(uint16_t x, uint16_t y, uint32_t color)
{
    ...
    DMA2D->OMAR = address;  // 目标存储器地址
    
    /* 设置传输尺寸：1个像素 */
    DMA2D->NLR = (1 << 16) | 1;  // 宽度=1，高度=1
    
    /* 启动传输 */
    DMA2D->CR |= DMA2D_CR_START;
    
    /* 可选：不等待传输完成，让DMA2D在后台运行 */
    // while (DMA2D->CR & DMA2D_CR_START);
}

/**
 * @brief       修复的DMA2D颜色填充函数 - 基于已验证的地址计算
 * @param       sx,sy: 起始坐标
 * @param       ex,ey: 结束坐标  
 * @param       color: 要填充的颜色数组首地址
 * @retval      无
...
/**
 * @brief       使用DMA2D填充矩形（带颜色数组/图像数据）不可用，图像有撕裂
 * @param       sx, sy: 起始坐标
 * @param       ex, ey: 结束坐标  
 * @param       colors: 颜色数组首地址
 * @retval      无
 */
...
//    /* 配置DMA2D - 存储器到存储器模式 */
//    DMA2D->CR = DMA2D_M2M;
//    DMA2D->FGPFCCR = LCD.ColorMode;  // 前景层颜色格式
//    DMA2D->FGMAR = (uint32_t)colors; // 源数据地址（颜色数组）
//    DMA2D->FGOR = 0;                 // 前景层行偏移（连续数据）
//    
//    DMA2D->OPFCCR = LCD.ColorMode;   // 输出颜色格式
//    
//    /* 计算目标地址 */
//    uint32_t address;
//    if(LCD.Direction == Direction_H) {  // 横屏
//        address = LCD.LayerMemoryAdd + LCD.BytesPerPixel * (LCD_Width * sy + sx);
//        DMA2D->OOR = LCD_Width - width;  // 行偏移
//    } else {  // 竖屏
//        address = LCD.LayerMemoryAdd + LCD.BytesPerPixel * ((LCD_Height - ex - 1) * LCD_Width + sy);
//        DMA2D->OOR = LCD_Width - height;  // 竖屏模式下行偏移
//    }
//    
//    DMA2D->OMAR = address;  // 目标存储器地址
//    
//    /* 设置传输尺寸 */
//    DMA2D->NLR = (width << 16) | height;  // 宽度=width，高度=height
//    
//    /* 启动传输 */
//    DMA2D->CR |= DMA2D_CR_START;
//    
